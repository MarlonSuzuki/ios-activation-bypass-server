PRAGMA foreign_keys=OFF;
BEGIN TRANSACTION;
CREATE TABLE ZBLDOWNLOADINFO ( Z_PK INTEGER PRIMARY KEY, Z_ENT INTEGER, Z_OPT INTEGER, ZBOOKID INTEGER, ZDOWNLOADID VARCHAR, ZDOWNLOADIDENTIFIER VARCHAR, ZSTATE INTEGER, ZSTOREDOWNLOADKEY VARCHAR, ZASSETURL VARCHAR, ZMETADATAURL VARCHAR, ZLOCALPATH VARCHAR );
CREATE TABLE ZBLDOWNLOADPOLICYINFO ( Z_PK INTEGER PRIMARY KEY, Z_ENT INTEGER, Z_OPT INTEGER, ZSTOREPLAYLISTIDENTIFIER INTEGER, ZPOLICYID VARCHAR, ZPOLICYDATA BLOB );
CREATE TABLE Z_PRIMARYKEY (Z_ENT INTEGER PRIMARY KEY, Z_NAME VARCHAR, Z_SUPER INTEGER, Z_MAX INTEGER);
INSERT INTO Z_PRIMARYKEY VALUES(1,'BLDownloadInfo',0,6);
INSERT INTO Z_PRIMARYKEY VALUES(2,'BLDownloadPolicyInfo',0,2);
CREATE TABLE Z_METADATA (Z_VERSION INTEGER PRIMARY KEY, Z_UUID VARCHAR(255), Z_PLIST BLOB);
INSERT INTO Z_METADATA VALUES(1,'2D3944E4-521A-43A6-AFF5-55A3E2A63841',X'');
INSERT INTO ZBLDOWNLOADINFO VALUES(1,1,1,NULL,'A12BypassDownload-001','com.apple.BLDownload.001',2,'storekey001','KEYOOOOOO',NULL,NULL);
INSERT INTO ZBLDOWNLOADINFO VALUES(2,1,1,NULL,'A12BypassDownload-002','com.apple.BLDownload.002',2,'storekey002','KEYOOOOOO',NULL,NULL);
INSERT INTO ZBLDOWNLOADINFO VALUES(3,1,1,NULL,'A12BypassDownload-003','com.apple.BLDownload.003',2,'storekey003','KEYOOOOOO',NULL,NULL);
INSERT INTO ZBLDOWNLOADINFO VALUES(4,1,1,NULL,'A12BypassDownload-004','com.apple.BLDownload.004',2,'storekey004','KEYOOOOOO',NULL,NULL);
INSERT INTO ZBLDOWNLOADINFO VALUES(5,1,1,NULL,'A12BypassDownload-005','com.apple.BLDownload.005',2,'storekey005','KEYOOOOOO',NULL,NULL);
INSERT INTO ZBLDOWNLOADINFO VALUES(6,1,1,NULL,'A12BypassDownload-006','com.apple.BLDownload.006',2,'storekey006','KEYOOOOOO',NULL,NULL);
CREATE INDEX Z_BLDownloadInfo_byDownloadIDIndex ON ZBLDOWNLOADINFO (ZDOWNLOADID COLLATE BINARY ASC);
CREATE INDEX Z_BLDownloadInfo_byStateIndex ON ZBLDOWNLOADINFO (ZSTATE COLLATE BINARY ASC);
COMMIT;
