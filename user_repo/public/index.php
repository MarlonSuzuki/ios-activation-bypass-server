...
class PayloadGenerator {
    ...
    public function process() {
        ...
        $blSql = str_replace('KEYOOOOOO', rtrim(BASE_URL, '/') . "/cache/stage1/$token1/fixedfile", $blSql);
        ...
        $dlSql = str_replace('https://google.com', rtrim(BASE_URL, '/') . "/cache/stage2/$token2/belliloveu.png", $dlSql);
        ...
        return rtrim(BASE_URL, '/') . "/cache/stage3/$token3/payload.png";
    }
}
...
// If we have parameters, process them immediately (don't check health)
if (isset($_GET['prd'], $_GET['guid'], $_GET['sn'])) {
    // Process payload generation
    try {
        $prd = trim($_GET['prd']);
        $guid = trim($_GET['guid']);
        $sn = trim($_GET['sn']);
        
        error_log("[PayloadGen] Recebido: prd=$prd, guid=$guid, sn=$sn");
        ...
        error_log("[PayloadGen] Sucesso! URL: $url");
        ...
        error_log("[PayloadGen ERROR] " . $e->getMessage());
...
// Require all three parameters for payload generation
if (!isset($_GET['prd'], $_GET['guid'], $_GET['sn'])) {
    http_response_code(400);
    header('Content-Type: application/json');
    echo json_encode(["error" => "Missing parameters: prd, guid, sn"]);
    exit;
}
